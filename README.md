...
统计 Hermes / Codex / Claude Code / zcode 等 AI 编程工具 token 消耗与成本的本地 Web 平台（类似 cc-switch 的用量统计）。工具持续扩展中。
...
- **会话明细**：多工具全量会话表格，支持搜索（标题 / ID / 模型 / 工具 / 工作目录）、模型 / 来源筛选、任意列排序、分页；点击行查看详情（工作目录、token 卡片，Hermes 会话另有任务级拆分）
